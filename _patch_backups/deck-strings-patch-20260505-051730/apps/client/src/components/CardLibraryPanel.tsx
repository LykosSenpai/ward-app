...
export function CardLibraryPanel({
  ...
  const magicTypes = useMemo(
    () => getUniqueValues(cardLibrary.map(card => card.magicType)),
    [cardLibrary]
  );
  ...
  const deckStats = useMemo(() => {
    ...
    return {
      ...
      lightningMagicCount: magicCards.filter(card => card.magicType === "LIGHTNING").length,
      averageAL
    };
  ...
  const filteredCards = useMemo(() => {
    ...
    return cardLibrary
      .filter(card => {
        ...
        if (magicTypeFilter !== "ALL" && `${card.magicType ?? ""}` !== magicTypeFilter) return false;
                    ...
                    <div className="library-card-content-grid library-option-a-card-content">
                      ...
                      <div className="library-card-main-info">
                        <div className="library-card-main static-library-card-main">
                          ...
                          <span>{card.cardType === "CREATURE" ? `${card.creatureType ?? "Creature"}  -  AL ${card.armorLevel}  -  SPD ${card.speed}  -  HP ${card.hp}  -  ${card.attackDice}D6 MOD ${card.modifier}` : `${card.magicType}  -  ${card.magicSubType}`}</span>
                        </div>
